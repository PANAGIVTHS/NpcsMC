...
$execute as @e[tag=phantom_driver] if score @s phantom.id = @e[tag=phantom_syncing,limit=1] phantom.id if score @s phantom.state matches 3 at @e[tag=phantom_syncing,limit=1] if entity @e[tag=target_$(id),distance=..24] facing entity @e[tag=target_$(id),limit=1,sort=nearest] feet run tp @e[tag=phantom_syncing,limit=1] ~ ~ ~ ~ ~
